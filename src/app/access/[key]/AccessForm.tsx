...
import {
  Briefcase, CheckCircle2, AlertCircle, Loader2, Eye, EyeOff, ShieldCheck,
} from "lucide-react";
...
export default function AccessForm({ accessKey }: { accessKey: string }) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      if (data.error === "already_exists") {
        // Compte existant → tente la connexion
        const { error: signInErr } = await supabase.auth.signInWithPassword({ email, password });
        if (signInErr) {
          setRegisterError("Un compte existe déjà avec cet email. Vérifiez votre mot de passe.");
      ...
      // Nouveau compte → connexion automatique
      const { error: signInErr } = await supabase.auth.signInWithPassword({ email, password });
      if (signInErr) {
        setRegisterError("Compte créé mais connexion échouée : " + signInErr.message);
        setRegistering(false);
        return;
      }
    }
  ...
  function renderField(field: FormField) {
    ...
    if (field.type === "textarea") return (
      <textarea value={val} onChange={(e) => setVal(e.target.value)} required={field.required}
    ...
    if (field.type === "radio" && field.options) return (
      <div className="flex flex-wrap gap-2 mt-1">
        {field.options.map((o) => (
          <label key={o} className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm cursor-pointer transition-colors ${val === o ? "bg-indigo-50 border-indigo-300 text-indigo-800" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
            <input type="radio" name={field.id} value={o} checked={val === o} onChange={() => setVal(o)} required={field.required} className="accent-indigo-600" />
            {o}
          </label>
        ))}
      </div>
    ...
    // text / email / file / inspiration → input text
    return (
      <input type={field.type === "email" ? "email" : "text"} value={val}
        onChange={(e) => setVal(e.target.value)} required={field.required}
    ...
    <div className="min-h-screen bg-gray-50 flex items-start justify-center py-12 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          <p className="text-sm text-gray-500 mb-6">Créez votre accès pour remplir le formulaire de votre agence.</p>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button type="submit" disabled={registering}
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed mt-1">
              {registering ? <><Loader2 size={14} className="animate-spin" />Création du compte...</> : "Créer mon accès"}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          <p className="text-gray-500 text-sm mb-8">
            Remplissez ces informations pour que nous puissions démarrer votre projet dans les meilleures conditions.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" disabled={submitting}
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white py-3 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed mt-2">
